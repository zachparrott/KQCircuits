...
def _get_excitations(json_data):
    return {["excitation"] for layer in json_data["layers"].values() if layer.get("excitation", 0) > 0}
...
if result_files:
    ...
    for key, result_file in zip(parameter_values.keys(), result_files):
        ...
        cmatrix[key] = {f"C{i+1}{j+1}": c for i, layer in enumerate(cdata) for j, c in enumerate(layer)}
