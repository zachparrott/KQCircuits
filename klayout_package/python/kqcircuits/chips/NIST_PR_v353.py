...
@add_parameters_from(Chip,
                     ...
                     frames_dice_width=['200'], frames_marker_dist = [250,250],
                     name_brand='NIST', name_chip='T1 PR', name_copy='v3')

class NISTpart353(Chip):
    ...
    qubit_center = Param(pdt.TypeDouble, "JJ position from center line", 1200, unit="μm") 
    qubit_separation = Param(pdt.TypeDouble, "Qubits XY offset from eachother measured to JJ.", 1500, unit="μm") 
    coupler_shift = Param(pdt.TypeDouble, "Resonator coupling offset from center line.", 300, unit="μm") 
    ...
    def _produce_readout_hangers(self, coupling_lengths):
        ...
        self.insert_cell(WaveguideComposite,
                            nodes = [
                                ...
                                Node((self.refpoints["W_port"].x + port_protection, self.refpoints["HR_U0_port_a"].y)),
                                Node((self.refpoints["HR_U0_port_a"].x - coupler_protection, self.refpoints["HR_U0_port_a"].y)),
                                Node(self.refpoints["HR_U0_port_a"])
                            ])
        ...
        self.insert_cell(WaveguideComposite,
                            nodes = [
                                ...
                                Node((self.refpoints["E_port"].x - port_protection, self.refpoints["HR_D0_port_a"].y)),
                                Node((self.refpoints["HR_D0_port_a"].x + coupler_protection, self.refpoints["HR_D0_port_a"].y)),
                                Node(self.refpoints["HR_D0_port_a"])
                            ])

        self.insert_cell(WaveguideComposite,
                            nodes = [
                                Node(self.refpoints["HR_U0_port_b"]),
                                Node((self.refpoints["HR_U0_port_b"].x + coupler_protection, self.refpoints["HR_U0_port_b"].y)),
                                Node((2600, self.refpoints["HR_U0_port_b"].y)),
                                Node((2600, self.refpoints["HR_D0_port_b"].y)),
                                Node((self.refpoints["HR_D0_port_b"].x - coupler_protection, self.refpoints["HR_D0_port_b"].y)),
                                Node(self.refpoints["HR_D0_port_b"])
                            ])
    ...
    def build(self):
        ...
        jj_cell = self.add_element(Overlap2, noSQUID=True, bridge_gap=self.bridge_gap, 
                                   finger_width=self.finger_width, pad_to_pad_separation=10, 
                                   hook_thickness = self.Q_hook_thickness,
                                   hook_lead_thickness = 0.2,
                                   )
        self.insert_cell(jj_cell, U_coords_jj[0], "jjU")
        self.insert_cell(jj_cell, D_coords[0], "jjD")
