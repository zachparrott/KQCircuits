...
from kqcircuits.qubits.NIST_logo_qubit import NISTLogo
...
@add_parameters_from(Chip,
                     ...
                     frames_dice_width=['200'], name_brand='NIST', name_chip='T1 PR', name_copy='v3')

class NISTpart353Logo(Chip):
    # parameters accessible in PCell
    a_launcher = Param(pdt.TypeDouble, "Pad CPW trace center", 250, unit="μm")
    ...
    launcher_indent = Param(pdt.TypeDouble, "Chip edge to pad port", 800, unit="μm") 
    qubit_center = Param(pdt.TypeDouble, "JJ position from center line", 1300, unit="μm") 
    qubit_separation = Param(pdt.TypeDouble, "Qubits XY offset from eachother measured to JJ.", 1500, unit="μm") 
    coupler_shift = Param(pdt.TypeDouble, "Resonator coupling offset from center line.", 200, unit="μm") 
    finger = Param(pdt.TypeBoolean, "IDC finger or 20 um for qubit 1", False)

    bridge_gap = Param(pdt.TypeDouble, "Junction bridge width.", 0.15, unit="μm")
    ...
    def _produce_readout_hangers(self, coupling_lengths):
        ...
        for i,clength in enumerate(coupling_lengths):
            if i % 2:
                name_prefix = "HR_D"
                trans = pya.DTrans(2, True, 2600 - 650 + self.qubit_separation / 2 + i*650 + 1 * (clength + self.r), 2600 + self.coupler_shift)
            else:
                name_prefix = "HR_U"
                trans = pya.DTrans(0, True, 2600 - self.qubit_separation / 2 + i*650 + -1 * (clength + self.r), 2600 - self.coupler_shift)
        ...
        self.insert_cell(WaveguideComposite,
                            nodes = [
                                ...
                                Node((self.refpoints["W_port"].x + port_protection, self.refpoints["W_port"].y)),
                                Node((self.refpoints["W_port"].x + port_protection, self.refpoints["HR_U0_port_sim_a"].y)),
                                Node((self.refpoints["HR_U0_port_sim_a"].x - coupler_protection, self.refpoints["HR_U0_port_sim_a"].y)),
                                Node(self.refpoints["HR_U0_port_sim_a"])
                            ])
        ...
        self.insert_cell(WaveguideComposite,
                            nodes = [
                                ...
                                Node((self.refpoints["E_port"].x - port_protection, self.refpoints["E_port"].y)),
                                Node((self.refpoints["E_port"].x - port_protection, self.refpoints["HR_D0_port_sim_a"].y)),
                                Node((self.refpoints["HR_D0_port_sim_a"].x + coupler_protection, self.refpoints["HR_D0_port_sim_a"].y)),
                                Node(self.refpoints["HR_D0_port_sim_a"])
                            ])

        self.insert_cell(WaveguideComposite,
                            nodes = [
                                Node(self.refpoints["HR_U0_port_sim_b"]),
                                Node((self.refpoints["HR_U0_port_sim_b"].x + coupler_protection, self.refpoints["HR_U0_port_sim_b"].y)),
                                Node((2600, self.refpoints["HR_U0_port_sim_b"].y)),
                                Node((2600, self.refpoints["HR_D0_port_sim_b"].y)),
                                Node((self.refpoints["HR_D0_port_sim_b"].x - coupler_protection, self.refpoints["HR_D0_port_sim_b"].y)),
                                Node(self.refpoints["HR_D0_port_sim_b"])
                            ])
    ...
    def _produce_d_res(self, i, length, adjust):
        self.insert_cell(
                ...
                nodes = [
                    ...
                    Node((self.refpoints[f"HR_D{i}_port_resonator_b"].x, self.refpoints[f"Q_D{i}_port_cplr_corner"].y), 
                         length_before=(length - adjust)),
    ...
    def build(self):
        ...
        if self.finger:
            res_lengths2 = [4179.8, 4171.4]
            coupling_lengths = [ 114., 112.]
        else:
        ...
        # top row positioning
        U_coords = [pya.DTrans(2, False, 2600 - self.qubit_separation / 2 + i * 1300, 2600 + self.qubit_center) for i in range(1)]
        U_coords_jj = [pya.DTrans(0, False, 2600 - self.qubit_separation / 2 + i * 1300, 2600 + self.qubit_center) for i in range(1)]
        # bottom row positioning
        D_coords = [pya.DTrans(0, False, 2600 + self.qubit_separation / 2 + i * 1300, 2600 - self.qubit_center) for i in range(1)]
        D_coords_jj = [pya.DTrans(0, False, 2600 + self.qubit_separation / 2 + i * 1300 + 239.734499, 2600 - self.qubit_center + 32.6596041) for i in range(1)]

        jj_cell = self.add_element(Overlap2, noSQUID=True, bridge_gap=self.bridge_gap, finger_width=self.finger_width, pad_to_pad_separation=10)
        self.insert_cell(jj_cell, U_coords_jj[0])
        self.insert_cell(jj_cell, D_coords_jj[0])
        ...
        self.insert_cell(
            NISTLogo, D_coords[0], f"Q_D0", ground_gap=['1200', '710'], 
            ground_gap_r=0.0, coupler_extent=['90', '20'], coupler_r=0.0, coupler_offset=50.0, 
            pads_height = 260, logo_gap = 20, pads_width = 707.652,
            _parameters='{"junction_type": "Sim"}', with_squid=False,
            island1_taper_width=0, island1_taper_junction_width=0, 
            island2_taper_width=0, island2_taper_junction_width=0, 
        )
        ...
        res_params= {
            ...
            'D0': {'len': res_final[1], 'adj': 960 + coupling_lengths[1] - rad_adj},
