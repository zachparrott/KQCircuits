...
class NISTpart35(Chip):
    ...
    def _produce_readout_hangers(self, coupling_lengths):
        ...
        for i,clength in enumerate(coupling_lengths):
            if i % 2:
                name_prefix = "HR_D"
                trans = pya.DTrans(2, True, 3000 + i*650 + 1 * (clength + self.r), 2600)
            else:
                name_prefix = "HR_U"
                trans = pya.DTrans(0, True, 1550 + i*650 + -1 * (clength + self.r), 2600)
    ...
    def _produce_u_res(self, i, length, adjust):
        self.insert_cell(
                ...
                nodes = [
                    ...
                    Node((self.refpoints[f"HR_U{i}_port_resonator_b"].x, self.refpoints[f"Q_U{i}_port_cplr_corner"].y - 95)),
                    Node((self.refpoints[f"Q_U{i}_port_cplr_corner"].x, self.refpoints[f"Q_U{i}_port_cplr_corner"].y - 95)),
        ...
        self.insert_cell(
                ...
                nodes = [
                    ...
                    Node((self.refpoints[f"HR_D{i}_port_resonator_b"].x, self.refpoints[f"Q_D{i}_port_cplr_corner"].y + 245), 
                         length_before=(length - adjust)),
                    Node((self.refpoints[f"HR_D{i}_port_resonator_b"].x, self.refpoints[f"Q_D{i}_port_cplr_corner"].y + 95)),
                    Node((self.refpoints[f"Q_D{i}_port_cplr_corner"].x, self.refpoints[f"Q_D{i}_port_cplr_corner"].y + 95)),
    ...
    def build(self):
        ...
        # top row positioning
        U_coords = [pya.DTrans(2, False, 2750 + i * 1300, 4150) for i in range(1)]
        # bottom row positioning
        D_coords = [pya.DTrans(0, False, 2450 + i * 1300, 1050) for i in range(1)]
        ...
        rad_adj = 3 * self.r * (2 - pi / 2)

        # 20 um 735, rounded 845, idc 515

        res_params= {
            
            'U0': {'len': res_final[0], 'adj': 2405 + coupling_lengths[0] - rad_adj},
