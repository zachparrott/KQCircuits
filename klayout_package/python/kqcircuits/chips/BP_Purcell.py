...
class PurcellQubits(Chip):
    ...
    filter_positions = Param(pdt.TypeList, "Fractional position of resonators along the bandpass resonator length.", [0.3, 0.433, 0.566,])
    resonator_lengths = Param(pdt.TypeList, "Readout resonators length in um.", [9000, 9000, 9000, ])
    resonator_couplings = Param(pdt.TypeList, "Readout coupling length in um.", [100, 100, 100, ])
    ...
    def build(self):
        ...
        # trans=pya.DTrans(0, True, 1000, 3750)

        # self.insert_cell(WaveguideCoplanar, path=pya.DPath([
        #     self.refpoints["W_port"], self.refpoints["E_port"]
        # ], 0))

        self.insert_cell(WaveguideCoplanar, path=pya.DPath([
            self.refpoints["W_port"], self.refpoints["Cin_port_a"]
        ], 0))
        ...
        if not self.filter_only:
            ...
            if len(self.filter_positions) < 2:
                ...
                for i in range(len(self.filter_positions)-1):
                    if i % 2:
