...
@add_parameters_from(Chip,
                     ...
                     frames_dice_width=[200,200], name_brand='IDC', name_chip='QQQ', 
...
class ChipQQQv2PIDC(Chip):
    ...
    def produce_junctions(self, penguin, **kwargs):
        ...
        for i, trans in enumerate(penguin):
            if i == 1:
                self.insert_cell(junction_cell, trans * pya.DTrans(0, False, self.jj_pos_A[0], self.jj_pos_A[1]), "jj_Ag")
                self.insert_cell(junction_cell, trans * pya.DTrans(0, False, self.jj_pos_B[0], self.jj_pos_B[1]), "jj_Bg")
                self.insert_cell(junction_cell, trans * pya.DTrans(0, False, self.jj_pos_C[0], self.jj_pos_C[1]), "jj_Cg")
            else:
                self.insert_cell(short_cell, trans * pya.DTrans(0, False, self.jj_pos_A[0], self.jj_pos_A[1]), "jj_Agt")
                self.insert_cell(short_cell, trans * pya.DTrans(0, False, self.jj_pos_B[0], self.jj_pos_B[1]), "jj_Bgt")
                self.insert_cell(short_cell, trans * pya.DTrans(0, False, self.jj_pos_C[0], self.jj_pos_C[1]), "jj_Cgt")



    def produce_squid(self, penguin, **kwargs):
        junction_cell = self.add_element(OverlapSimple,
                                         pad_to_pad_separation = 8.0, **kwargs)
        ...
        self.insert_cell(junction_cell, penguin[1] * pya.DTrans(0, False, self.jj_pos_SR[0], self.jj_pos_SR[1]), "jj_SRt")
        self.insert_cell(short_cell, penguin[0] * pya.DTrans(0, False, self.jj_pos_SR[0], self.jj_pos_SR[1]), "jj_SRg")
    ...
    def build(self):
        ...
        qubit_cell = self.layout.create_cell("QQQv2_PIDC", Qubit.LIBRARY_NAME)
        self.insert_cell(qubit_cell, pya.DTrans(0, False, 2600, 2600), "QQQv2PIDC")
        ...
        for trans in penguin_translations:
        ...
        self.insert_cell(resolution, pya.DTrans(0, False, 1670-250, 470), "res")
        
        self.insert_cell(Profilometer, pya.DTrans(0, False, 1670-112.5-250, 345), "pro")

        NISTlogo = self.layout.create_cell("NISTlogo", Element.LIBRARY_NAME)
        
        # measured from crap GDS logo file
        scaleFactor = 40 / 158.016
        self.insert_cell(NISTlogo, pya.DCplxTrans(scaleFactor, 0, False, 3800, 550), "logo")

        produce_label(self.cell, "Z.Parrott", pya.DPoint(3475, 550-125), LabelOrigin.TOPLEFT, 0, 0, 
                      [self.face()["base_metal_gap_wo_grid"]], self.face()["ground_grid_avoidance"], 50)
