...
class JunctionTestQCCDouble(Chip):
  ...
  def build(self):
    ...
    jj_pos_A = pya.DTrans(0, False, -70, 158.6035 + 6)
    jj_pos_B = pya.DTrans(0, False, 70, 158.6035 + 6)
      ...
      for ri, t in enumerate(trow):
        ...
        for k, jt in enumerate(jj_trans):
          if k < 1:
            ...
            if ti < 3:
              self.insert_cell(array_cell, t * jt * pya.DTrans(0, False, -(aWidth*1e-3 + self.array_gap)/2, 0), f"{ti}{ri}{k}")
              self.insert_cell(array_cell, t * jt * pya.DTrans(0, False, +(aWidth*1e-3 + self.array_gap)/2, 0), f"{ti}{ri}{k}2")
            else:
              self.insert_cell(array_short,t * jt * pya.DTrans(0, False, -(aWidth*1e-3 + self.array_gap)/2, 0), f"{ti}{ri}{k}")
              self.insert_cell(array_short,t * jt * pya.DTrans(0, False, +(aWidth*1e-3 + self.array_gap)/2, 0),f"{ti}{ri}{k}2")
