...
class OverlapSeries(Junction):
    ...
    includeTop = Param(pdt.TypeBoolean, "Include Top landing", True)
    includeBot = Param(pdt.TypeBoolean, "Include bottom landing", True)
    ...
    def _make_squid_junction(self): #, top_corner, b_corner_y, finger_margin=0):
        ...
        connect_pts = [
            pya.DPoint(shift_finger - self.finger_width / 2, - self.pad_to_pad_separation / 2 - self.pad_offset - 1.5),
            pya.DPoint(shift_finger - self.finger_width / 2, - self.pad_to_pad_separation / 2 - self.pad_offset - 0.5),
            pya.DPoint(shift_hook + self.taper_width / 2, - self.pad_to_pad_separation / 2 - self.pad_offset - 0.5),
            pya.DPoint(shift_hook + self.taper_width / 2, - self.pad_to_pad_separation / 2 - self.pad_offset - 1.5),
        ]

        dep_pts = [hook_pts, finger_pts, connect_pts]
        junction_shapes = [pya.DPolygon(pts).to_itype(self.layout.dbu) for pts in dep_pts]
        self._add_shapes(junction_shapes, "SIS_junction")
    ...
    def _produce_ground_metal_shapes(self):
        ...
        if self.include_base_metal_gap:
            if self.include_base_metal_addition:
                if self.includeBot and not self.includeTop:
                    pts = bottom_pts + [
                        pya.DPoint(-self.width/2, -self.pad_to_pad_separation/2),
                        pya.DPoint(-self.width/2, self.pad_to_pad_separation/2),
                        pya.DPoint(self.width/2, self.pad_to_pad_separation/2),
                        ]
                elif not self.includeBot and self.includeTop:
                    pts = top_pts[::-1] + [
                        pya.DPoint(self.width/2, self.pad_to_pad_separation/2),
                        pya.DPoint(self.width/2, -self.pad_to_pad_separation/2),
                        pya.DPoint(-self.width/2, -self.pad_to_pad_separation/2),
                        ]
                elif not self.includeBot and not self.includeTop:
                    pts = [pya.DPoint(-self.width / 2, -self.metal_height/2), pya.DPoint(self.width / 2, -self.metal_height/2),
                           pya.DPoint(self.width / 2, self.metal_height/2), pya.DPoint(-self.width / 2, self.metal_height/2)]
                else:
